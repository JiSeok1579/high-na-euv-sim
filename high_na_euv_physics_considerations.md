# High-NA EUV Lithography Digital Twin: 물리 법칙 및 프로젝트 고려사항 정리

## 0. 문서 목적

이 문서는 **High-NA EUV Lithography Machine을 직접 복제하는 설계서가 아니라**,  
연구/교육/시뮬레이션 목적의 **High-NA EUV 리소그래피 디지털 트윈 또는 노광 시뮬레이터**를 구축할 때 고려해야 할 물리 법칙, 수학 모델, 공정 지식, 구현 우선순위를 정리한 문서이다.
...
```text
High-NA EUV 노광 현상을 근사하는 디지털 트윈 구축
```
