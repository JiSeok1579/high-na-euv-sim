# High-NA EUV Lithography Simulator — 프로젝트 총정리

> 작성일: 2026-04-25
> 위치: `~/Desktop/High-NA EUV Lithography Simulator/`
...
> **0.55 NA EUV 노광 장비(ASML EXE 시리즈)에서 13.5 nm EUV 빛이 광원 → 조명계 → 반사형 마스크 → 6-mirror anamorphic projection → wafer를 거쳐 형성되는 aerial image와 printed pattern을 Fourier optics + ray tracing + mask 3D + photoresist 모델로 근사하는 연구·교육용 시뮬레이터를 만든다.**
...
1. **학습 (Understand)** ★ 1차 — 각 Phase의 의미, 흐름, 좋은 결과 기준을
   정성적으로 파악.
2. **시각화 (Visualize)** ★ 1차 — 3D notebook으로 결과를 입체적으로 확인.
3. **탐색 (Explore)** — NA, sigma, dose, focus 등 입력 변수 sweep으로 거동 관찰.
4. **공유 (Share)** — 21편 논문의 핵심 거동을 코드로 명시화. 목표는 정성 일치.
...
단위 혼용, 감사 로그 부정직처럼 학습을 망치는 항목만 P0/P1로 유지한다.
...
│
├── REVIEWER_DIRECTIVE.md            ← Claude 외부 감사자 지침 (v2.1)
...
│     • Layer 2 — external/ 외부 감사 (Claude, REVIEWER_DIRECTIVE v2.1)
...
├── docs/study_grade_relaxation.md
│     • Study purpose, relaxed audit severity, and workspace rename guidance
│
├── scripts/
...
| **Phase 3** Wafer topography & DOF | 4주 | `wafer_topo.py` (defocus phase) | #14, #5 | DOF = k₂·λ/NA² formula/trend 검증 |
...
- ✅ `src/ilt.py` — Phase 6 Part 03: full-layout contour EPE, finite-difference mask-bias gradient, and ILT refinement
- ✅ `src/opc.py` — Phase 6 Part 04: assist-feature OPC candidates, arbitrary mask contour evaluation, and pixel-level ILT refinement
- ⏳ `src/` (Phase 6 follow-up) — polygon contour extraction, grayscale/autograd ILT, and external solver coupling
...
- ✅ `tests/phase6_ilt.py` — Phase 6 Part 03 full-layout contour EPE, finite-difference gradient, and ILT refinement tests
- ✅ `tests/phase6_opc.py` — Phase 6 Part 04 assist-feature OPC, arbitrary mask evaluation, and pixel-level ILT tests
- ⏳ `tests/` (Phase 6 follow-up) — polygon contour, grayscale/autograd, and external-solver tests
...
- ✅ `notebooks/5_SMO_PMWO.ipynb` — Phase 6 SMO/PMWO/OPC/ILT candidate ranking and optimized print comparison
...
### 6.2.1 Remaining Work Checklist

| Area | Current status | Remaining work | Study-grade estimate |
|------|----------------|----------------|----------------------|
...
│   ├── phase6_ilt.py                 # numeric-gradient ILT and contour EPE
│   └── phase6_opc.py                 # assist-feature OPC and pixel ILT
├── data/
...
### Step 2 — Phase 4 rigorous-data replacement
...
### Step 3 — Phase 6 polygon/autograd ILT refinement
- Extend binary pixel-level ILT to polygon contour extraction and grayscale/autograd candidate families.
- Add larger source/pupil/wavefront sweeps and external solver comparisons.
...
```
WHAT       0.55 NA EUV lithography simulator (research/education)
WHY        High-NA의 회절·M3D·DOF·stochastic·SMO design space 탐색
WHERE      ~/Desktop/High-NA EUV Lithography Simulator/
...
자산 (현재):
  ...
  ✅ Simulator code Phase 1/2/3/5/6 Part 04 implemented
...
다음 단계:
  1. Final report + API documentation pass
...
- **2026-04-26** Phase 5 MVP (Threshold Resist) 구현 완료.
  - `src/resist_threshold.py`, `src/metrics.py` 작성
  - `tests/phase5_threshold.py`, `tests/integration_end_to_end.py` 추가
  - `notebooks/4a_threshold_resist.ipynb`, `docs/phase5_resist_models.md` 추가
  - KPI K1 end-to-end MVP 합격: mask → aerial/DOF → threshold resist → CD/EPE
- **2026-04-26** Phase 5 L1 Part 01 (Gaussian Blur Resist) 구현 완료.
  - `src/resist_blur.py`, `tests/phase5_blur.py`, `notebooks/4b_resist_levels.ipynb` 추가
  - Gaussian kernel normalization, constant-dose preservation, edge-spread broadening, blurred end-to-end CD tolerance 검증
- **2026-04-26** Phase 5 L1 Part 02 (Blur Dose Sweep) 구현 완료.
  - `blur_dose_sweep`으로 sigma-dose별 CD/EPE/transition width/LWR proxy table 추가
  - LWR proxy sigma 증가/ dose 증가 방향성 회귀 검증
- **2026-04-26** Phase 5 L2 Part 01 (Depth-Resolved Resist) 구현 완료.
  - `src/resist_depth.py`, `tests/phase5_depth.py` 추가
  - top/bottom dose asymmetry, Beer-Lambert attenuation, focus-depth coupling, Phase 3 focus-drilling consistency 검증
- **2026-04-26** Phase 5 L2 Part 02 (Profile/SWA Proxy) 구현 완료.
  - `depth_cd_profile`, `sidewall_angle_proxy` 추가
  - depth별 CD profile, top-bottom CD delta, deterministic SWA proxy 회귀 검증
...
- **2026-04-26** Phase 5 완료 (Calibration + K6 Exit) 구현 완료.
  - `calibrate_blur_lwr_proxy`, `calibrate_sidewall_angle_proxy`, `calibrate_stochastic_lwr_budget` 추가
  - L1 LWR, L2 SWA, L3 stochastic LWR measured-input calibration hook 및 KPI K6 exit 상태 검증
- **2026-04-27** MT-015/MT-016 + Phase 4 entry implemented.
  - Project naming updated to High-NA EUV Lithography Simulator and `.github/CLAUDE.md` synced to v2.1 English-first guidance.
  - Added Tier 1 matplotlib 3D notebooks for focus stack, pupil wavefront, resist depth dose, and Mask 3D effect bars.
  - Added `src/mask_3d.py`, starter absorber data, `tests/phase4_m3d_6effects.py`, and `docs/phase4_M3D_design.md` for paper #12 six-effect stubs.
- **2026-04-27** Phase 4 Part 02 boundary-correction fields implemented.
  - Added `boundary_corrected_mask`, `BoundaryCorrectionResult`, and `load_absorber_materials_json`.
  - Added tests for zero-CRA Kirchhoff preservation, asymmetric shadow maps, boundary phase strength, secondary ghost fields, and measured-row JSON loading.
- **2026-04-27** Phase 4 Part 03 absorber provenance + lookup-table hooks implemented.
  - Added `Mask3DLookupEntry`, `Mask3DLookupTable`, `AbsorberScreeningRow`, lookup JSON loading, pitch interpolation, reduced-model fallback, and candidate screening.
  - Added provenance fields to absorber JSON and a qualitative lookup schema example for future rigorous data replacement.
- **2026-04-27** Phase 4 Part 04 rigorous-data import + aerial regression hooks implemented.
  - Added lookup CSV import, lookup-driven boundary correction, aerial-image regression metrics, and a header-only rigorous import template.
  - Added regression coverage for imported nm columns, lookup override behavior, and RMSE/max-error aerial gates.
- **2026-04-27** INST-001 study-purpose relaxation applied.
  - Declared study purpose in README, PROJECT_OVERVIEW, and 진행계획서.
  - Added `docs/study_grade_relaxation.md` and synchronized `REVIEWER_DIRECTIVE.md` / `.github/CLAUDE.md` with study-grade audit severity.
...
- **2026-04-27** Phase 6 Part 01 SMO MVP implemented.
...
- **2026-04-27** Phase 6 Part 03 ILT numeric-gradient refinement implemented.
  - Added `src/ilt.py` with full-layout contour EPE, finite-difference mask-bias gradients, and bounded ILT refinement.
  - Added `tests/phase6_ilt.py` and expanded `notebooks/5_SMO_PMWO.ipynb`.
  - Updated Phase 6 docs to record the Part 03 simplifications and verification scope.
- **2026-04-27** Phase 6 Part 04 assist-feature OPC and pixel-level ILT implemented.
